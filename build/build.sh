...
# ── (و-4ب) وجهةُ خادمِ النفق: **مُجمَّدةٌ حتّى يُرفَع الأثر** [BR-05] ──
# ‏bin/mihrab-tunnel.exe يحتاج وجهتَين مخبوزتَين وقتَ الترجمة (option_env!):
#   • VSCODE_CLI_UPDATE_ENDPOINT   ⇒ {قاعدة}/{جودة}/{نظام}/{معماريّة}/<لاحقة>
#   • VSCODE_CLI_DOWNLOAD_ENDPOINT ⇒
#     {قاعدة}/download/{name}/{app}-reh-web-{نظام}-{معماريّة}-{name}.tar.gz
# و patch_cli_endpoints.py جعل تصديرَهما **شرطيًّا** في build_cli.sh، فما لا
# نُصدِّره هنا يعطي option_env! القيمةَ None ويردّ الـCLI «no update url» صراحةً
# (وهي أوّلُ بوّابةٍ يبلغها: `get_latest_commit` تنادي `get_update_endpoint`
# قبل التنزيل — update_service.rs:75. و«no download url» لا تُرى إلّا لو
# بُكِّرت وجهةُ التحديث وحدَها، وهي حالةٌ يمنعها هذا السكربتُ لأنّه يصدّرهما معًا).
#
# ⚠️ **والافتراضُ مُطفَأ، والقرارُ ملكيٌّ لا هندسيّ.** تفعيلُ القناة **لا رجعةَ فيه
#    عمليًّا**: العنوانُ يُخبَز في البايتات ولا مستودعَ نُسَخٍ لثنائيّ الـCLI عندنا،
#    فكلُّ ثنائيٍّ يُشحَن بعد التفعيل يشير إلى ذلك المضيف **أبدًا**؛ وإسقاطُ المسار
#    لاحقًا يكسر ثنائيّاتٍ مشحونةً بصمت. ويجرّ معه استضافةً بلا سياسةِ استبقاءٍ
#    مكتوبة، وقناةَ توزيعٍ ثانيةً غيرَ موصوفةٍ في سياسة التوقيع. فحتّى يُحسَم ذلك:
#    السلسلةُ محفوظةٌ في الشيفرة ومطفأةٌ في السلوك.
#
...
# ‏**والوجهةُ تُنزَع لا يُكتفى بألّا تُصدَّر.** التصديرُ في `build_cli.sh` مشروطٌ على
# `${VAR:-}` **الموروثة من البيئة**، فمن ضبطها في صَدَفته أو في CI كان يخبزها في
# الثنائيّ بينما هذا السكربتُ يطبع «لا تُخبَز وجهةُ النفق». والبوّابةُ في آخر الملفّ
# كانت تفحص `releases/download` وحدَها فلا ترى مضيفًا آخر — أي أنّ الكتلةَ المكتوبةَ
# لإغلاق التسريب كانت تتركه مفتوحًا من الباب الذي سمّته بنفسها.
unset VSCODE_CLI_DOWNLOAD_ENDPOINT VSCODE_CLI_UPDATE_ENDPOINT

if [[ "$MIHRAB_TUNNEL" != "yes" ]]; then
  log "سلسلةُ النفق مُجمَّدة (‏MIHRAB_TUNNEL=$MIHRAB_TUNNEL) — لا تحزيمَ ولا قياسَ ولا خبز"
elif [[ -z "$MIHRAB_SERVER_BASE" ]]; then
  ...
  # ‏**شكلُ العنوان يُشتقّ من الشجرة المُرقَّعة لا يُكتَب بيد.** رقعةُ ويندوز
  # `41-cli-fix-update-url.patch` تُقحِم مقطعَ `user/` في مسار المانيفست، فمسارٌ
  # مكتوبٌ بيدٍ من رقعةِ REH وحدَها يقيس عنوانًا لا يطلبه العميلُ على منصّتنا الأولى.
  _SHAPE="$("$PY_BIN" "$ROOT/build/cli_url_shape.py" \
            "$UP/vscode/cli/src/update_service.rs")" || {
    echo "❌ تعذّر اشتقاقُ شكل عناوين الـCLI من الشجرة المُرقَّعة." >&2
    echo "   ولا يُخمَّن: عنوانٌ صحيحُ الشكل خاطئُ المسار يبدو مُصلَحًا ويردّ 404." >&2
    exit 1; }
  _MAN_SUFFIX="$(printf '%s\n' "$_SHAPE" | sed -nE 's/^MANIFEST_SUFFIX=(.*)$/\1/p')"
  [[ -n "$_MAN_SUFFIX" ]] || { echo "❌ لاحقةُ المانيفست فارغةٌ بعد الاشتقاق." >&2; exit 1; }

  _CLI_APP="$(node -p "require(process.argv[1]).applicationName" "$UP/product.json")" || {
    echo "❌ تعذّرت قراءةُ applicationName من $UP/product.json" >&2; exit 1; }
  [[ -n "$_CLI_APP" && "$_CLI_APP" != "undefined" ]] || {
    echo "❌ applicationName = ${_CLI_APP:-—} — واسمُ الأثر يُشتقّ منه." >&2
    echo "   ولولا هذا الفحصُ لطُلِب undefined-reh-web-… فردّ 404، ووقع الاتّهامُ" >&2
    echo "   على الناشر والعطبُ عندنا." >&2
    exit 1; }

  _LATEST="$MIHRAB_SERVER_BASE/stable/$_CLI_OS/$VSCODE_ARCH/$_MAN_SUFFIX"
  # ── أرضيّةُ صدقٍ للمِجَسّ: **عجزُه يُميَّز عن غيابِ الأثر** ──
  # ‏أوّلُ صياغةٍ كتبت أنبوبًا واحدًا `curl … | jq -r '.name'` وقرأت الفراغَ «لا
  # مانيفست» — وتقول ذلك حرفيًّا حين **يغيب `jq`**، أي رسالةٌ تصف الخادمَ وهي
  # تصف جهازَ البناء. فالحالاتُ لا تُخلَط:
  #   • 404          ⇒ لا مانيفستَ منشور. الحالةُ الصحيحةُ ما لم يُرفَع الأثر.
  #   • 000 أو 5xx   ⇒ **عجزتُ عن السؤال** لا «ليس منشورًا»: شبكةٌ · وكيلٌ · خادمٌ
  #                    يُعاد تحميلُه. وقراءتُها «غيرَ منشور» تشحن ثنائيًّا بلا نفقٍ
  #                    بسببٍ مكتوبٍ خاطئ، والبناءُ exit=0 والبوّابةُ خضراء.
  #   • 200 بلا اسم  ⇒ عطبٌ عندنا: `jq` أو شكلُ المانيفست.
  # ‏**و`|| true` لازمةٌ على الإسناد لا ممنوعة**: `curl -f` يخرج بغير صفرٍ على 404
  # والملفُّ تحت `set -e` — فبدونها يسقط البناءُ على الحالة الصحيحة نفسِها. والرمزُ
  # يبقى مقروءًا لأنّ `-w` يُطبَع حتّى مع `-f`.
  _MAN="$(mktemp)"
  _MAN_CODE="$(curl -fsS -o "$_MAN" -w '%{http_code}' --max-time 20 "$_LATEST" 2>/dev/null || true)"
  case "${_MAN_CODE:-000}" in
    000|5??)
      rm -f "$_MAN"
      echo "❌ تعذّر قياسُ مانيفست الخادم (رمز ${_MAN_CODE:-000}): $_LATEST" >&2
      echo "   وهذا **عجزٌ عن السؤال** لا جوابٌ بالنفي — شبكةٌ أو وكيلٌ أو خادمٌ" >&2
      echo "   يُعاد تحميلُه. ولو قُرِئ «غيرَ منشور» لشُحِن ثنائيٌّ بلا نفقٍ بسببٍ" >&2
      echo "   مكتوبٍ خاطئ. أعِد المحاولةَ، أو MIHRAB_TUNNEL=no لتخطّي السلسلة." >&2
      exit 1 ;;
  esac
  if [[ "$_MAN_CODE" != "200" ]]; then
    rm -f "$_MAN"
    log "لا مانيفستَ خادمٍ (رمز $_MAN_CODE) — لا تُخبَز وجهةُ النفق"
    echo "   العنوان: $_LATEST" >&2
    echo "   المعنى: tunnel و serve-web سيردّان «no update url» عند المستخدم." >&2
    echo "   وهو **مقصودٌ** ما لم يُنشَر الأثر (يُطبَع أمرُ النشر بعد التحزيم)." >&2
    _REL_NAME=""
  else
...
if [[ -d "${_REH_DIRS[0]:-}" ]]; then
  ...
  # ── (ط-0د1) والأثرُ يُحزَم — **حين تُطلَب السلسلة** [BR-05] ──
  # هذه الشجرةُ نفسُها هي ما ينزّله `mihrab-tunnel` ويشغّله. والدَّينُ لم يكن في
  # المصدر ولا في الثنائيّ بل في **خطوةِ تحزيمٍ غائبة**؛ وهي الآن موجودةٌ ومطفأة:
  # ‏132 م.ب و≈17 ثانيةً على جهاز البناء ثمنٌ لا يُدفَع في كلّ بناءٍ ما دام الأثرُ
  # لا يُرفَع، ولا شيءَ يكنس `$UP/assets` فيتراكم إلى الأبد.
  #
  ...
  # هناك. ولهذا يُقرأ `applicationName` من `product.json` ولا يُكتب بيدٍ.
  #
  # والمحتوى **من جذر الشجرة لا من مجلّدها**: الـCLI يفكّ الأرشيفَ ثمّ يبحث عن
  # `bin/<serverApplicationName>` — **وعلى ويندوز يضيف `.cmd`** (options.rs:48-50).
  # ولهذا يقبل الفحصُ الثلاثةَ: بلا لاحقة · `.cmd` · `.sh`، **بمِرساةِ نهاية**:
  # بادئةٌ وحدَها تقبل `mihrab-server-old` وتشهد لأرشيفٍ لا يُقلِع.
  if [[ "$MIHRAB_TUNNEL" != "yes" ]]; then
    log "لا يُحزَم أثرُ الخادم (‏MIHRAB_TUNNEL=$MIHRAB_TUNNEL) — راجع (و-4ب)"
  else
  case "$OS_NAME" in
  ...
  if [[ -z "$_PKG_OS" ]]; then
    ...
    for _c in "$WEB_DIR/bin/$_PKG_SRV" "$WEB_DIR/bin/$_PKG_SRV.cmd" "$WEB_DIR/bin/$_PKG_SRV.sh"; do
      # ‏`if` لا «[[ … ]] && …» — **وليس لـ`set -e`**. جُرّبت الصيغتان: شرطٌ
      # كاذبٌ في آخر دورةٍ **لا يُنهي السكربت**، لأنّ bash يعفي ما يسبق الطرفَ
      # الأخيرَ في قائمة `&&`. فالسببُ قراءةٌ لا أكثر، ويُقال صراحةً: تعليلٌ
      # يدّعي خطرًا لا يقع يُعلّم قارئَه قاعدةً خاطئة.
      if [[ -f "$_c" ]]; then _PKG_ENTRY="$_c"; break; fi
    done
    [[ -n "$_PKG_ENTRY" ]] || {
      echo "❌ لا نقطةَ دخولٍ في $WEB_DIR/bin/ باسم $_PKG_SRV[.cmd|.sh] — الـCLI" >&2
      echo "   يبحث عنها بعد فكّ الأرشيف، فيُخفِق بعد تنزيل مئات الميغابايتات." >&2
    ...
    #
    # ⚠️ **والفهرسُ يُكتَب ثمّ يُقرأ — ولا يُمرَّر في أنبوب.** أوّلُ صياغةٍ كتبت
    #    `tar tzf … | grep -q …` فأسقطت البناءَ **لحظةَ نجاحها**: `grep -q` يخرج
    #    عند أوّل تطابقٍ فيغلق الأنبوبَ، فيموت `tar` بـSIGPIPE، و`pipefail`
    #    يعطي الأنبوبَ حالةَ 141. قِيس على أرشيفٍ سليم: `PIPESTATUS = 141 0`.
    #    ولم يمسكها إلّا بناءٌ كامل، لأنّ القياسَ اليدويَّ جرى بلا `pipefail`.
    #
    # وبادئةُ `./` **اختياريّةٌ في المِرساة**: GNU tar يكتبها مع `tar czf … .`،
    # و`bsdtar` (‏macOS وبعضُ ويندوز) قد لا يكتبها — فمِرساةٌ تفرضها تحمرّ على
    # أرشيفٍ سليمٍ بُني بأداةٍ أخرى.
    _REH_LIST="$(mktemp)"
    tar tzf "$REH_TGZ" > "$_REH_LIST" 2>/dev/null || {
      rm -f "$_REH_LIST"
      echo "❌ أرشيفُ الخادم لا يُقرأ بعد كتابته: $REH_TGZ" >&2; exit 1; }
    grep -qE "^(\./)?bin/$_PKG_SRV(\.cmd|\.sh)?$" "$_REH_LIST" || {
      echo "❌ أرشيفُ الخادم بلا bin/$_PKG_SRV[.cmd|.sh] في جذره — يُفكّ سليمًا ولا يُقلِع." >&2
      echo "   ما في جذر bin/: $(grep -E "^(\./)?bin/[^/]+$" "$_REH_LIST" | tr '\n' ' ')" >&2
      rm -f "$_REH_LIST"; exit 1; }
    rm -f "$_REH_LIST"
    log "أثرُ الخادم جاهز: $(du -m "$REH_TGZ" | cut -f1) م.ب · $(basename "$REH_TGZ")"
    if [[ -z "$_CLI_ENDPOINT" ]]; then
      # ‏**الأمرُ يُطبَع هنا لا في (و-4ب)**: تلك تجري قبل التحزيم بمئتَي سطر،
      # فمن فتح `assets/` لحظتَها وجده فارغًا. والمسارُ مبسوطٌ لا `$UP` حرفيّة:
      # المتغيّرُ يعيش في السكربت لا في صَدَفة القارئ، وسطرٌ وُضِع ليُلصَق يجب أن يُلصَق.
      echo "   ولا وجهةَ نفقٍ مخبوزةٌ في هذا البناء. لنشر الأثر:" >&2
      echo "     build/publish_server.sh \"$REH_TGZ\"" >&2
      echo "   وبعده: البناءُ التالي يقيسه فيخبز الوجهةَ من نفسه." >&2
    fi
  fi
  fi
else
...
if true; then
  ...
  if [[ -n "${_CLI_ENDPOINT:-}" ]]; then
    ...
    # ‏**ويُقاس مضيفُنا كذلك، لا `releases/download` وحدَه.** التعليقُ كان يقول
    # «لا `releases/download` ولا مضيفُنا» ويفحص الأوّلَ فقط — أي حارسٌ يدّعي
    # في نصّه ما لا يفعله في شرطه. ومصدرُ الثاني حقيقيّ: `build_cli.sh` يصدّر
    # `${VAR:-}` الموروثةَ من البيئة، فمن ضبطها في CI أو في صَدَفته خبزها.
    # (ونُزِعت في (و-4ب) — وهذا يقيس أنّ النزعَ بلغ الثنائيّ فعلًا.)
    for _leak in "releases/download" "$MIHRAB_SERVER_BASE"; do
      [[ -n "$_leak" ]] || continue
      LC_ALL=C grep -qa "$_leak" "$_TUNNEL" && {
        echo "❌ [BR-05] وجهةٌ مخبوزةٌ في ثنائيّ النفق: $_leak" >&2
        echo "   ولم تُصدَّر وجهةٌ في (و-4ب) — فمصدرُها ليس نحن: رقعةٌ منبعيّةٌ أو" >&2
        echo "   قيمةٌ ورثها build_cli.sh من البيئة. وثنائيٌّ يشير إلى مضيفٍ" >&2
        echo "   لم يُقَس أنّه يخدم يُنزِّل شيئًا ويشغّله على جهاز المستخدم." >&2
        exit 1; }
    done
  fi
