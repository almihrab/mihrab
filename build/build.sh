...
[[ -f "$_TUNNEL" ]] || {
  ...
  exit 1; }
if true; then
  # ‏**ولا وجهةَ تنزيلٍ إطلاقًا.** كانت البوّابةُ تطلب وجهتَنا، ثمّ قِيس أنّ الاسمَ
  # الذي وجّهتْ إليه **غيرُ مسجَّلٍ على GitHub** — فصارت تشترط أن يشير الثنائيُّ إلى
  # ما يستطيع أيُّ أحدٍ تملّكَه. والعقدُ الآن: لا `releases` مخبوزةٌ في الثنائيّ حتّى
  # يُنشَر أثرُ REH فعلًا. وحينها يُعاد السطران معًا: التصديرُ في (و-4) والطلبُ هنا.
  LC_ALL=C grep -qa "releases/download" "$_TUNNEL" && {
    echo "❌ [BR-05] وجهةُ تنزيلٍ مخبوزةٌ في ثنائيّ النفق: ${_TUNNEL#$OUTDIR/}" >&2
    echo "   ولا أثرَ REH يُحزَم ولا يُرفَع — فالوجهةُ وعدٌ بلا خلفيّة، وعنوانٌ لا" >&2
    echo "   نملكه يُنزَّل منه ويُشغَّل خادمًا على جهاز المستخدم. راجع (و-4)." >&2
    exit 1; }
