...
if [[ -d "${_REH_DIRS[0]:-}" ]]; then
  ...
  if [[ -z "$_PKG_OS" ]]; then
    ...
    #    يعطي الأنبوبَ حالةَ 141. قِيس على أرشيفٍ سليم:
    #        PIPESTATUS = 141 0   ⇐ grep طابق، وtar قُتِل، والحكمُ «لم يطابق»
    #    فتقول الرسالةُ إنّ الأرشيفَ بلا نقطةِ دخولٍ وهي فيه. وهي الفئةُ عينُها
    #    التي وُثِّقت في بوّابة `vscodium` أسفلَ هذا الملفّ، وقعتُ فيها ثانيةً —
    #    ولم يمسكها إلّا بناءٌ كامل، لأنّ قياسي اليدويَّ جرى بلا `pipefail`.
    _REH_LIST="$(mktemp)"
    tar tzf "$REH_TGZ" > "$_REH_LIST" 2>/dev/null || {
      rm -f "$_REH_LIST"
      echo "❌ أرشيفُ الخادم لا يُقرأ بعد كتابته: $REH_TGZ" >&2; exit 1; }
    grep -q "^\./bin/$_PKG_SRV" "$_REH_LIST" || {
