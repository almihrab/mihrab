...
for v in "$@"; do
  ...
  #
  # ── ومقياسُ البوّابة **أوّلُ نشرٍ للملفّ**، لا «اليومُ دائمًا» ──
  # كانت تشترط تاريخَ اليوم على كلّ ملفٍّ في كلّ نداء، فأبطلت شيئين:
  #   • الاستعمالَ الموصوفَ في ترويسة هذا السكربت (`… 1.126 1.125`) — لكلّ إصدارٍ
  #     تاريخُه، فيستحيل أن يحمل الاثنان تاريخًا واحدًا.
  #   • **وإعادةَ النشر كلَّها** — وهذا السكربتُ نفسُه ينتهي برسالةٍ تقول «أصلِح كتلةَ
  #     nginx ثمّ أعِد النشر». من فعل ذلك غدًا تردُّه البوّابة، فلا يبقى أمامه إلّا
  #     تزييفُ سطر التاريخ: العطبُ عينُه الذي وُضِعت لمنعه.
  # فتُسأل الوجهةُ أوّلًا: ملفٌّ ليس هناك ⇒ هذا أوّلُ نشرٍ ويجب أن يكون التاريخُ اليومَ.
  # ملفٌّ موجودٌ ⇒ إعادةُ نشرٍ، وتاريخُ الإصدار الأصليُّ هو الصحيح فلا يُمَسّ.
  #
  # والمستقبلُ مرفوضٌ في الحالتين: تاريخٌ لم يأتِ بعدُ كذبٌ مهما كان سببُ النشر.
  #
  # والبوّابةُ ترفض ولا تُصحّح: كتابةُ التاريخ في الملفّ تُبقي المصدرَ والمنشورَ نسخةً
  # واحدة، وتصحيحٌ صامتٌ عند الرفع يجعلهما نسختين تفترقان بلا أثر.
  if "${SSH[@]}" "$HOST" "test -f '$DEST/$name'" 2>/dev/null; then PUBLISHED=yes; else PUBLISHED=no; fi
  "$PY_BIN" - "$path" "$name" "$PUBLISHED" <<'PYDATE' || exit 1
import io, re, sys, datetime
path, name, published = sys.argv[1], sys.argv[2], sys.argv[3]
...
# يومان مقبولان: المحلّيُّ والعالَميّ. جهازُ بناءٍ شرقَ غرينتش بعد منتصف الليل يرى
# يومًا والخادمُ يرى غيرَه — نافذةُ ساعاتٍ يحمرّ فيها نشرٌ صحيح، بلا عطبٍ تحتها.
local_today = datetime.date.today()
utc_today = datetime.datetime.now(datetime.timezone.utc).date()
if stamped > max(local_today, utc_today):
    sys.stderr.write(
        "\u274c %s يُعلِن تاريخًا في المستقبل: %s (اليوم %s)\n"
        "   تاريخٌ لم يأتِ بعدُ كذبٌ مهما كان سببُ النشر.\n" % (name, stamped, local_today))
    sys.exit(1)
if published != "yes" and stamped not in (local_today, utc_today):
    sys.stderr.write(
        "\u274c %s يُنشَر لأوّل مرّة بتاريخٍ غيرِ اليوم: %s (اليوم %s)\n"
        "   يومُ أوّلِ نشرٍ هو يومُ الإصدار — وهذا التاريخُ يصل القارئَ كاذبًا.\n"
        "   صحّحِ السطرَ في release-notes/%s ثمّ أعِد النشر.\n" % (name, stamped, local_today, name))
    sys.exit(1)
PYDATE
