...
CORE_DIFFS = [
    ...
    # الطرفيّةُ حيث لا طرفيّة [WEB-05]: بناؤنا الثابت بلا خادم ⇒ `processSupported` كاذب،
    # وكلُّ أوامر الطرفيّة مُقيَّدةٌ به سلفًا. لكنّ **وصفَ العرض** غيرُ مقيَّد، ومنه يشتقّ
    # `viewsService` لسانَ اللوحة وأمرَ الفتح (‏Ctrl+Backquote) وبندَ قائمة العرض — ثلاثةُ
    # مداخلَ من إغفالٍ واحد. والرابعُ قاعدةُ Ctrl+Shift+C في `terminal.web.contribution.ts`:
    # تُسجَّل على سجلّ الاختصارات مباشرةً فتتخطّى `precondition` الذي يحرس الأمرَ في كلّ
    # مكانٍ آخر. والخامسُ بندُ «طرفيّة جديدة» في قائمة الطرفيّة — إخوتُه الأربعةُ في الكتلة
    # نفسِها يحملون الشرطَ وهو وحدَه لا يحمله. والوعدُ الذي لا خلفيّةَ له أسوأُ من غياب
    # الميزة: لوحٌ يُفتَح ولا يُنتج شيئًا يُقرأ عطبًا في جهاز المستخدم لا حدًّا في المنتَج.
    "patches/core/034-terminal-unavailable-in-web.patch",
    # ‏**اتّجاهُ حقول الإدخال يتبع محتواها، وافتراضُه اتّجاهُ التطبيق.** الطبقةُ الثانية لا تكفي:
    # ‏`unicode-bidi: plaintext` يضبط **تصييرَ** النصّ ولا يمسّ `direction` العنصر، وذاك ما يقرّر
    # **جهةَ الفيض** — وفيضُ اليسار في صندوقٍ يساريٍّ لا يُمرَّر أصلًا، فاستعلامٌ عربيٌّ طويل يخرج
    # من الصندوق ولا يلاحقه المؤشّر (مقيس: `scrollWidth = clientWidth`). وقلبُ الاتّجاه بالـCSS
    # يقلب العطبَ إلى اللاتينيّ ولا يزيله. و`dir="auto"` وحدَه لا يكفي كذلك: ارتدادُه المنصوصُ
    # عليه في HTML هو **اليسار** حين لا يجد محرفًا قويًّا، فحقلٌ فارغٌ أو `123` أو `-->` يصير
    # يساريًّا في تطبيقٍ عربيّ. فالرقعةُ تحسب أوّلَ محرفٍ قويٍّ بنفسها وترتدّ إلى اتّجاه المستند.
    "patches/core/035-inputbox-content-direction.patch",
    # عنوانُ نصّ الملاحظات كان حرفيًّا في `releaseNotesEditor.ts`، فلا يبلغه مفتاحٌ في
    # الهويّة: بلا الرقعة يقرأ مستخدمُ محرابٍ ملاحظاتِ إصدارِ VS Code — إنجليزيّةً،
    # ويساريّةً، وتصف بناءً غيرَ الذي بين يديه ولا تذكر تعديلَ محرابٍ واحدًا. وتُورِث
    # الرقعةُ مستندَ العرضِ اتّجاهَ القشرة: الـwebview مستندٌ مستقلٌّ لا يرث شيئًا.
    "patches/core/036-release-notes-source.patch",
]
