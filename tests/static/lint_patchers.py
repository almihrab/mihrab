...
def _report_proposal_heads(text):
    """عناوينُ البنود: `[(الرقم، المحرفُ الذي يليه مباشرةً، الموضع)]`.

    **مستخرِجٌ واحدٌ يستهلكه حارسان** عمدًا. كانا يعرّفان الصيغةَ مرّتين — أحدُهما
    `find("## م-N)")` نصًّا والآخرُ تعبيرًا نمطيًّا — فانجرفا: عنوانٌ بشَرطةٍ سقط من
    الأوّل بصمت. تعريفان لعرفٍ واحدٍ ينجرفان حتمًا؛ فليكن التعريفُ هنا وحدَه.

    و**لا `\\s*` بين الرقم والمحرف**: `## م-٢٥ ) عنوان` بمسافةٍ واحدة كان يمرّ من فحص
    الصيغة (المسافةُ تُبتلَع فيُرى القوس) ولا يطابقه `find` — أي عينُ الثقب. و`(.?)`
    لا `(.)` كي يُلتقَط عنوانٌ عارٍ في آخر السطر بدل أن يفلت من الفحصين معًا.
    """
    return [(m.group(1), m.group(2), m.start())
            for m in re.finditer(r"(?m)^##[ \t]+م-([٠-٩]+)(.?)", text)]


def _report_table_nums(text):
    """أرقامُ البنود في الجدول الموجز، بترتيب ورودها (التكرارُ محفوظٌ عمدًا)."""
    i = text.find("## الجدول الموجز")
    j = text.find("\n## ", i + 1)
    rows = [ln for ln in text[i:j if j > 0 else len(text)].splitlines()
            if ln.lstrip().startswith("|")]
    return [m.group(1) for ln in rows for m in [re.search(r"\[م-([٠-٩]+)\]", ln)] if m]
...
def _upstream_report_rows_have_sections():
    ...
    """
    text = _read(UPSTREAM_REPORT)
    nums = _report_table_nums(text)
    assert len(nums) >= 10, f"لم أستخرج أرقامَ البنود من الجدول ({len(nums)}) — عمِيَ الفحص."
    pos = {n: k for n, nxt, k in _report_proposal_heads(text) if nxt == ")"}
    thin = []
    for n in nums:
        k = pos.get(n, -1)
        if k < 0:
...
@check("تقريرُ المنبع: رقمُ البند مفتاحٌ — لا تكرارَ ولا صيغةَ عنوانٍ شاذّة")
def _upstream_report_numbers_are_keys():
    """‏رقمُ البند هو ما نُحيل به إليه من الوثائق ومن رسائل الالتزام ومن الرُقَع. فإن
    حمَله بندان صار المرجعُ ملتبسًا، **والأسوأُ أنّ الحارسَ القائمَ يعمى عنده**:
    `_upstream_report_rows_have_sections` يبحث عن `## م-{n})` فيجد أوّلَ صاحبِ الرقم
    ويمرّ أخضرَ، بينما البندُ الثاني قد يكون بلا قسمٍ أصلًا. وهذا ما وقع فعلًا: بندان
    برقم `م-١٨`، وأحدُهما عنوانُه بشَرطةٍ لا بقوس (`## م-١٨ — …`) فما طابقه فحصُ
    المتن قطُّ — ومع ذلك كان اللوحُ كلُّه أخضر.

    ولذلك يؤكِّد هذا الفحصُ **الصيغةَ** كذلك لا التكرارَ وحدَه: عنوانُ بندٍ لا ينتهي
    رقمُه بقوسٍ هو عنوانٌ خارجَ مدى كلِّ حارسٍ يعتمد `## م-N)`، أي ثقبٌ صامتٌ في
    التغطية لا مسألةَ ذوقٍ في التنسيق.
    """
    text = _read(UPSTREAM_REPORT)
    # شاهدُ تفعيلٍ موجَب قبل أيّ توكيدٍ سالب: لو تغيّرت بنيةُ العناوين فلم نستخرج شيئًا
    # لصار «لا تكرار» صادقًا على الفراغ — وهو أخضرُ كاذبٌ من الصنف الذي نطارده هنا.
    # وشاهدان لأنّ العَرَضَ كان في **الجدول والعناوين معًا**؛ فحصُ العناوين وحدَه كان
    # يترك صفَّين برقمٍ واحدٍ يمرّان خضراوين، وهما نصفُ ما بُلِّغ.
    heads = _report_proposal_heads(text)
    table = _report_table_nums(text)
    assert len(heads) >= 10, (
        f"لم أستخرج عناوينَ البنود ({len(heads)}) — تغيّرت بنيةُ التقرير فعمِيَ الفحص.")
    assert len(table) >= 10, (
        f"لم أستخرج صفوفَ الجدول ({len(table)}) — تغيّرت بنيتُه فعمِيَ نصفُ الفحص.")

    odd = sorted({f"م-{n}" for n, nxt, _ in heads if nxt != ")"})
    assert not odd, (
        "عناوينُ بنودٍ بصيغةٍ لا تُطابِق `## م-N)` — وهي الصيغةُ التي تبحث عنها حرّاسُ "
        "التغطية، فالبندُ يبقى بلا فحصِ متنٍ وهو يبدو مغطّى: " + " · ".join(odd))

    nums = [n for n, _, _ in heads]
    dup = sorted({f"م-{n}" for n in nums if nums.count(n) > 1}
                 | {f"م-{n}" for n in table if table.count(n) > 1})
    assert not dup, (
        "رقمُ بندٍ يحمله عنوانان أو صفّان — المرجعُ يصير ملتبسًا وفحصُ المتن يعمى عن "
        "الثاني: " + " · ".join(dup))

    # والاتّجاهُ المعاكس: قسمٌ لا صفَّ له. `rows_have_sections` يمشي جدولًا ⇐ قسمًا وحده،
    # فبندٌ يُكتَب متنُه ويُنسى صفُّه يبقى غيرَ مذكورٍ في الجدول الموجز — وهو الجدولُ الذي
    # يقرؤه حارسُ تغطية الرُقَع. فيصير البندُ موجودًا ومحجوبًا في آن.
    orphan = sorted({f"م-{n}" for n in nums} - {f"م-{n}" for n in table})
    assert not orphan, (
        "أقسامُ بنودٍ بلا صفٍّ في الجدول الموجز — المتنُ مكتوبٌ والفهرسُ لا يذكره: "
        + " · ".join(orphan))
...
def _debug_console_defaults():
    ...
    assert isinstance(lh_px, (int, float)) and lh_px > 0, (
        ...
        "‎1.4‎»، وهو دون أرضيّة الحبر المقيسة ‎2.683em‎ فيُقصّ التشكيل [DG-01 · TY-02]")
...
def _line_height_and_gpu():
    ...
    **‏`editor.lineHeight`** مشتقٌّ من مدى الحبر العربيّ **المركَّب**، ويُقرأ من المصدر
    الواحد `tests/dx/arabic_ink.measured.json` لا من رقمٍ منسوخٍ هنا. والرقمُ السابق
    ‎1.798em‎ كان خطأً في الأساس: قِيس من كونتورات الرسوم منفردةً (‏«أ» U+0623 عند
    ‎+1.265‎ وتنوينُ الكسر U+064D عند ‎−0.533‎) فلم يركّب علامةً على قاعدةٍ قطّ، وGPOS
    يرفعها فوق ذلك. والمقيسُ حيًّا ‎2.683em‎. وافتراضُ المنبع ‎1.35‎ (ويندوز/لينكس) دونه
    بمرّتين. فالحدُّ **محسوبٌ لا مختار**.

    والحارسُ **ذو حدّين**: أرضيّةٌ تمنع القصّ، وسقفٌ يمنع الاتّجاهَ الآخر. حارسٌ أُحاديٌّ
    يدفع الرقمَ صعودًا بلا ثمنٍ مرئيّ، وكلُّ زيادةٍ فوق الحاجة أسطرٌ تخرج من الشاشة.
    ...
    **‏`experimentalGpuAcceleration`** يُقفَل على `off`: طبقةُ GPU المنبعيّة بلا أيّ معالجة
    ...
    """
    ink = json.load(open(os.path.join(ROOT, "tests", "dx", "arabic_ink.measured.json"),
                        encoding="utf-8"))
    MIN_LINE_HEIGHT = ink["composedInkEm"]   # الأرضيّة المقيسة — تحتها اقتطاعٌ مؤكَّد
    MAX_INK_HEADROOM = 1.10  # السقف: ‎10٪‎ فوق الحاجة المقيسة، فلا يُشترى الأمانُ بأسطر
    ...
    # فقيمةٌ تساوي الأرضيّةَ بالضبط تنكسر عند أحجامٍ بعينها. والمطلوبُ قيمةٌ تصمد **في كلّ
    # حجمٍ عمليّ** — وهذا سببُ اختيار ‎2.8‎ (‏= 42px عند ‎15‎ بلا كسر) دون ‎2.683‎ نفسِها.
    ...
    over = [s for s in FONT_SIZES if round(lh * s) / s > MIN_LINE_HEIGHT * MAX_INK_HEADROOM]
    assert not over, (
        f"‏`editor.lineHeight` = {lh} يتجاوز الحاجةَ المقيسة {MIN_LINE_HEIGHT}em بأكثرَ من "
        f"‏{round((MAX_INK_HEADROOM - 1) * 100)}٪ عند أحجام {over} — الأمانُ من القصّ يُشترى "
        f"هنا بأسطرٍ تخرج من الشاشة. الحارسُ ذو حدّين قصدًا: لولا السقفُ لَما مَنَعَ شيئًا "
        f"إلّا في اتّجاهٍ واحد [TY-02]")
...
def _font_size_measured():
    ...
    if isinstance(lh, (int, float)):
        floor = json.load(open(os.path.join(ROOT, "tests", "dx", "arabic_ink.measured.json"),
                              encoding="utf-8"))["composedInkEm"]
        assert round(lh * size) / size >= floor, (
            f"‏`lineHeight`={lh} مع `fontSize`={size} يعطي "
            f"{round(lh * size) / size:.3f}em بعد تقريب المنبع — دون الأرضيّة {floor} [TY-02]")
...
def _web_host_page_contract():
    ...
    # (ط) [WEB-10] وسومُ التبكير: **الطرفُ الذي يُقرأ من الصفحة وحدها.**
    #     الوجودُ في الشجرة يفحصه `patch_web_host.py --verify` لأنّه وحده يرى
    #     المبنيَّ؛ وما يلي يُقرأ هنا لأنّه خطأٌ في الصفحة نفسِها، ولأنّ فحصَ
    #     البناء لا يجري إلّا في بناءٍ كامل.
    #
    #     و`crossorigin` على تبكير الخطّ **ليست زينةً**: الخطوطُ تُجلَب في وضع
    #     CORS مجهولٍ حتّى من أصلِها، فتبكيرٌ بلا الرّاية لا يطابق طلبَ
    #     `@font-face` **فيُنزَّل الخطُّ مرّتَين**. وهو عطبٌ **يبدو ناجحًا**:
    #     الخطُّ يظهر، والصفحةُ تعمل، والكلفةُ وحدها تضاعفت — أي أنّ التحسينَ
    #     صار ضررًا بلا أثرٍ مرئيّ. وهذا أخبثُ من تبكيرٍ مكسورٍ يُرى.
    _pre = re.findall(
        r'<link\s(?=[^>]*rel="(?:module)?preload")[^>]*\shref="([^"]+)"', html_only)
    assert _pre, (
        "‏لا وسمَ تبكيرٍ في `web/index.html` — والفحصُ يقيس صفرًا. إن أُسقِط "
        "التبكيرُ عمدًا فأسقِط هذا الفحصَ معه بسببٍ مكتوب [WEB-10]")
    for _h in _pre:
        assert "://" not in _h and not _h.startswith("//"), (
            "‏تبكيرٌ إلى أصلٍ خارجيّ: " + _h + " — والصفحةُ تخدم من أصلِها وحده، "
            "وسياسةُ الأمان تحجبه فتدفع رحلةً لا تقبض شيئًا [CSP-01][WEB-10]")
    for _m in re.finditer(r'<link\s[^>]*\sas="font"[^>]*>', html_only):
        assert "crossorigin" in _m.group(0), (
            "‏تبكيرُ خطٍّ بلا `crossorigin` — الخطوطُ تُجلَب في وضع CORS مجهولٍ "
            "حتّى من أصلِها، فالتبكيرُ لا يطابق طلبَ `@font-face` ويُنزَّل الخطُّ "
            "مرّتَين. تحسينٌ يضاعف الكلفةَ ومظهرُه سليم [WEB-10]")
...
def _tunnel_server_chain_is_one_url():
    ...
    ‏`mihrab-tunnel` لا يقرأ إعدادًا ولا `product.json`: العنوانُ **مخبوزٌ في
    بايتاته** وقتَ الترجمة. فمن يبنيه (‏`build.sh` و-4ب) ومن يحزم الأثر
    (‏`build.sh` ط-0د1) ومن يرفعه (‏`publish_server.sh`) يجب أن يتّفقوا حرفيًّا.
    واختلافُهم **لا يُسقِط بناءً ولا يُحمِّر فحصًا**: يُنشَر كلُّ شيءٍ بنجاح، ثمّ
    يُخفِق النفقُ على جهازٍ لا نراه برسالةٍ لا تدلّ على السبب.

    وهذا **بالضبط** ما وقع مرّةً: صُحِّح المضيفُ وبقي اسمُ الأثر `vscodium`،
    فصار العنوانُ صحيحَ المضيف خاطئَ الأثر — يبدو مُصلَحًا في القراءة ويردّ 404.
    """
    b = _read(os.path.join(ROOT, "build", "build.sh"))
    pub = os.path.join(ROOT, "build", "publish_server.sh")
    assert os.path.isfile(pub), (
        "‏`build/publish_server.sh` غائب — والبناءُ يُخرِج أثرَ خادمٍ لا ناشرَ له، "
        "فتعود وجهةُ النفق وعدًا بلا خلفيّة [BR-05]")
    p = _read(pub)

    # (أ) القاعدةُ نفسُها في الموضعَين، مكتوبةً افتراضًا لا مفترضةً.
    _bases = set(re.findall(
        r'MIHRAB_SERVER_BASE[:-]?-(https?://[^"}\s]+)', b + "\n" + p))
    assert len(_bases) == 1, (
        "‏قاعدةُ خادمِ النفق تختلف بين مواضعها: " + repr(sorted(_bases)) +
        " — والعنوانُ مخبوزٌ في الثنائيّ، فالاختلافُ يُنشَر بنجاحٍ ويُخفِق عند "
        "المستخدم وحدَه [BR-05]")

    # (ب) والتحزيمُ يشتقّ الاسمَ ولا يكتبه: قِيس عطبٌ كتب `vscodium` حرفيًّا.
    assert 'require(process.argv[1]).applicationName' in b, (
        "‏اسمُ الأثر لا يُشتقّ من `product.json` في `build.sh` — وكتابتُه بيدٍ هي "
        "العطبُ الذي وقع فعلًا: مضيفٌ صحيحٌ وأثرٌ باسم المنبع [BR-05]")
    assert "-reh-web-" in b and "-reh-web-" in p, (
        "‏قالبُ اسم الأثر غائبٌ عن أحد الطرفَين — والقالبُ ليس اصطلاحًا عندنا بل "
        "ما يبنيه `update_service.rs` حرفيًّا [BR-05]")

    # (ج) **والوجهةُ لا تُصدَّر إلّا بعد قياسٍ حيّ.** أخطرُ انحدارٍ هنا أن يصير
    #     التصديرُ غيرَ مشروطٍ «لتبسيط السكربت» — فيعود ما أُغلِق: عنوانٌ مخبوزٌ
...
def _deploy_does_not_expose_editor():
    ...
    **(ب) `default_server`**: على الخادم المقيس لا كتلةَ `default_server` على 443،
    ...
    # ── ‏[CACHE-02] ‏`/out/` غيرُ مبصومٍ ⇒ عمرُ الطزاجة صفرٌ ──
    # الحارسُ فوقَه يمنع **يومًا فأكثر**، وعشرُ دقائقَ كانت تمرّ منه — وهي التي
    # أعطبت حيًّا: نُشِرت `sad-lang` ولم تُلوِّن عند المُبلِّغ، لأنّ قائمةَ الامتدادات
    # المضمَّنة مخبوزةٌ في الحزمة، فحزمةٌ عمرُها دقائقُ لا تعرف اللغةَ إطلاقًا.
    # أي أنّ الحارسَ السابقَ كان يقيس **الحدَّ الفادح** ويترك الفادحَ الصغير.
    #
    # ويُقاس العقدُ لا الإملاء: `no-cache` و`max-age=0, must-revalidate` و`no-store`
    # عقدٌ واحد، وأيُّها كُتِب يمرّ. والمقياسُ عمرُ الطزاجة = 0.
    _static = os.path.join(ngx_dir, "mihrab.dev.static.conf")
    if os.path.isfile(_static):
        _txt = _read(_static)
        _at = _txt.find("location /out/")
        assert _at != -1, (
            "‏كتلةُ `location /out/` غابت عن mihrab.dev.static.conf — ومساراتُها "
            "غيرُ مبصومة، فبلا كتلةٍ تُخدَم بافتراضِ nginx (بلا `Cache-Control` أصلًا) "
            "ويستدلّ المتصفّحُ على مدّةٍ من عنده.")
        _end = _txt.find("\n    }", _at)
        assert _end != -1, "‏كتلةُ /out/ بلا `}` على مستواها — تعذّر اقتطاعُها."
        _blk = _txt[_at:_end]
        _cc = [ln.split("#", 1)[0] for ln in _blk.splitlines()
               if "Cache-Control" in ln.split("#", 1)[0]]
        assert len(_cc) == 1, (
            "‏كتلةُ /out/ فيها " + str(len(_cc)) + " سطرَ `Cache-Control` — المنتظَر "
            "واحد. تعدُّدُها في المستوى نفسِه يجعل السارِيَ آخرَها بلا أن يظهر ذلك.")
        _v = _cc[0].lower()
        # ‏**تُنزَع القيمُ ذاتُ الوسائط قبل القراءة.** `no-cache="Set-Cookie"` **اسمُ
        # حقلٍ** لا تصفيرُ طزاجة، فترويسةُ `max-age=600, no-cache="Set-Cookie"` كانت
        # تمرّ من الشرط وعمرُها ستّمئة ثانية — والادّعاءُ أنّ الحارسَ «يقيس العقدَ
        # لا الكلمة» غيرُ محقَّقٍ ما دام يطابق سلسلةً فرعيّة.
        # و`\\"` لا `"` وحدَها: nginx يهرّب الاقتباسَ الداخليَّ داخل قيمةٍ مقتبَسة
        # (`"… no-cache=\\"Set-Cookie\\""`)، فتعبيرٌ يطلب اقتباسًا عاريًا لا يطابق
        # شيئًا — ويعود العطبُ كما كان. أمسكه المُصابُ لا القراءة.
        _v = re.sub(r'(no-cache|private)=\\?"[^"]*\\?"', "", _v)
        if "no-store" in _v or "no-cache" in _v:
            _fresh = 0
        else:
            _m2 = re.search(r"max-age\s*=\s*(\d+)", _v)
            _fresh = int(_m2.group(1)) if _m2 else -1
        assert _fresh == 0, (
            "‏عمرُ طزاجةِ /out/ = "
            + ("غيرُ محدَّد (فيستدلّ المتصفّحُ من عنده)" if _fresh < 0
               else str(_fresh) + " ثانية")
            + " — ومساراتُ /out/ **غيرُ مبصومة**، فكلُّ ثانيةِ طزاجةٍ **نافذةُ عمًى**: "
            "زائرٌ داخلَها لا يبلغه النشرُ الجديد أصلًا، وحزمتُه تحمل قائمةَ "
            "الامتدادات المخبوزةَ القديمة. اكتب `no-cache` (أو `max-age=0, "
            "must-revalidate`). والمراجعةُ رخيصةٌ مقيسة: ‏١٠ طلبات في الإقلاع البارد، "
            "وnginx يردّ 304 بصفرِ بايت.")
...
def _patcher_anchors_do_not_eat_cr():
    ...
    ‏`\\r?$` في مِرساةٍ تُستعمَل للاستبدال **يبتلع محرفَ الإرجاع**، والبديلُ لا
    يُعيده — فيخرج سطرٌ بـLF في ملفٍّ كلُّه CRLF. ولا يُسقِط بناءً ولا يُحمِّر نحوًا:
    يتعثّر فيه git والمحرِّراتُ لاحقًا، ويُنسَب العطبُ إلى غير سببه.

    ووقع مرّتين: في `patch_cli_endpoints.py` وهو يُكتَب، وفي `patch_dev_build_env.py`
    قبله — **وتعليقُ الأخير كان يدّعي أنّ L1 يجرّبه على النهايتَين**. وهو ادّعاءٌ
    كاذب: المرقِّعُ في `BUILD_PATCHERS` وL1 يتخطّاها بحكم تعريفه (لا مصدرَ منبعٍ
    نظيفًا لها). أي أنّ الفئةَ التي صار لها اسمٌ وحارسٌ [CRLF-01] بقيت بلا تغطيةٍ
    في نصفِ المرقِّعات، والتعليقُ سدَّ مكانَ الحارس.

    والقاعدةُ صياغةٌ لا اجتهاد: نهايةُ السطر تُرى ولا تُؤخَذ — `(?=\\r?$)`.
    """
    bad = []
    for name in sorted(os.listdir(BUILD)):
        if not (name.startswith("patch_") and name.endswith(".py")):
            continue
        for i, raw in enumerate(_read(os.path.join(BUILD, name)).splitlines(), 1):
            # ‏**التعليقُ ليس شيفرة.** أوّلُ صياغةٍ قرأت السطرَ خامًا فحمّرت نفسَها على
            # التعليق الذي يشرح القاعدة — وهو الفخُّ الذي وقع فيه [WEB-06] حرفيًّا.
            # و`_code` وحدَها لا تكفي هنا: مبنيّةٌ لتجريد تعليقات JS لا `#` بايثون.
            line = raw.split("#", 1)[0]
            if "r?$" not in line:
                continue
            # النظرةُ الأماميّةُ هي الصيغةُ المقبولة، وهي الوحيدة.
            if "(?=" in line and line.index("(?=") < line.index("r?$"):
                continue
            bad.append(name + ":" + str(i) + "  " + line.strip()[:80])
    assert not bad, (
        "‏[CRLF-02] مِرساةٌ تستهلك نهايةَ السطر — تبتلع محرفَ الإرجاع ولا تُعيده، "
        "فيخرج الملفُّ مختلطَ النهايات:\n   " + "\n   ".join(bad)
        + "\n   اجعلها نظرةً أماميّةً: (?=\\r?$)")
