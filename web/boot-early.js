...
(function () {
  ...
  // ‏**والزائرُ العائدُ لا يُنزِّل شيئًا** — فلا يُقال له «تُحمَّل ٥ م.ب».
  // ‏`/out/` و`/` كلاهما `no-cache`: كلُّ ملفٍّ يُراجَع مشروطًا فيردّ الخادمُ 304
  // بصفرِ بايت. فالانتظارُ عنده رحلاتُ ذهابٍ وإياب لا تنزيل — وعلى شبكةٍ عالية
  // الكمون يبلغ ثانيتَين أو ثلاثًا، أي يتجاوز عتبةَ الثلاث ثوانٍ أدناه فيقرأ
  // رسالةً **كاذبةً في حالته**. وهو عينُ ما يحذّر منه هذا الملفُّ في الشريط الذي
  // «يكذب بالإيحاء»: رسالةٌ كُتبت بصدقٍ لحالةٍ واحدةٍ تصير كذبًا في الأخرى.
  //
  // والعلامةُ تُقرأ وتُكتَب في `try` — التخزينُ المحلّيُّ يرمي في نافذةٍ خاصّةٍ أو
  // بإعداداتٍ تمنع بياناتِ المواقع، ورميُه هنا يقتل الإقلاعَ كلَّه قبل أن يبدأ.
  var returning = false;
  try { returning = !!localStorage.getItem('mihrab.web.booted.v1'); } catch (e) { }

  // مراحلُ صادقةٌ بلا ادّعاءِ نسبة: لا نعرف كم نزل، فنقول ما نعرف فقط.
  // ⚠️ **والعلامةُ تجيب «أزارَنا من قبل؟» لا «أعنده هذه الحزمةُ بعينها؟».** وبينهما
  //    ثلاثُ طرق: نشرٌ جديدٌ يردّ 200 بالبايتات كاملة، وذاكرةُ HTTP تُطرَد (شجرةٌ
  //    بهذا الحجم من أوّل المرشَّحين)، ومستخدمٌ يمسح «الملفّاتِ المخزَّنة» ويُبقي
  //    بياناتِ المواقع — وهو الافتراضيُّ في حوار كروم. فنفيُ التنزيل صراحةً يكذب
  //    في أشيع الحالات لا أندرِها. والصفحةُ لا تعرف ما تغيّر قبل أن تنزّله، فتُقال
  //    الحالتان معًا بدل الجزم بإحداهما.
  var STAGES = returning ? [
    [3000,  'نتحقّق ممّا تغيّر منذ زيارتك الأخيرة — وما لم يتغيّر لا يُنزَّل ثانية.'],
    [15000, 'أطولُ من المعتاد — إمّا نُشِر بناءٌ جديدٌ يُنزَّل الآن، وإمّا الاتّصالُ بطيء.'],
    [60000, 'ما زلنا ننتظر الخادم. الانتظارُ ما زال مُجديًا.'],
  ...
  function ok() {
    ...
    // ‏**تُعلَّم بعد النجاح لا قبله.** علامةٌ تُكتَب عند بدء الإقلاع تجعل زائرًا
    // سقط إقلاعُه أوّلَ مرّةٍ يُعامَل «عائدًا» في المحاولة التالية، فيُقال له إنّ
    // الحزمةَ عنده سلفًا وهي ليست عنده.
    try { localStorage.setItem('mihrab.web.booted.v1', '1'); } catch (e) { }
  }
  ...
  addEventListener('error', function (ev) {
    ...
    // ‏**تحميلٌ مسبقٌ تلميحٌ لا مورد.** سقوطُه لا يُعطِّل شيئًا: المستهلكُ الحقيقيُّ
    // يفشل بنفسه ويُلتقَط (الاستيرادُ الديناميُّ عبر `unhandledrejection`، والورقةُ
    // عبر وسمها). وبلا هذا السطر يقلب `preload` عطبًا **غيرَ قاتل** إلى شاشةِ فشلٍ
    // فوق ورشةٍ تعمل — ولهذا كان تبكيرُ الخطّ مُسقَطًا قبل هذا السطر،
    // **وبه أُعيد** (‏`index.html`: تبكيرُ `kawkab-mono.woff2` بعد قياس). فيبقى الشرطُ
    // كي لا يُعاد فتحُ البابِ بوسمٍ جديدٍ يومًا.
    if (t && t.rel && (t.rel === 'preload' || t.rel === 'modulepreload')) { return; }
