...
_got_conf="$(_sha "$STAGE/mihrab.dev.static.conf")"
[[ "$_got_conf" == "$SHA_CONF" ]] || die "بصمةُ كتلة nginx لا تطابق المثبَّتة في هذا السكربت.
   المعنى: هذه الكتلةُ ليست التي غادرت جهازَ البناء — وهي ما يُقلِع nginx أو يُسقِطه.
   المثبَّتة: $SHA_CONF
   المقيسة : $_got_conf
   ▸ حمولةٌ جديدة؟ اكتب البصمةَ من جهاز البناء: deploy/stage_payload.sh --nginx-only <خادم>
   ▸ وإلّا فلا تُحدِّثها:  sudo rm -rf $STAGE  ثمّ أعِد الرفع."

if [[ $NGINX_ONLY == no ]]; then
  [[ -f $STAGE/tree.tgz ]] || die "لا شجرةَ في $STAGE/tree.tgz"
  # ‏**رسالةٌ للحالة النادرة تُقرأ في الحالة الشائعة.** كُتِبت لتبديلِ الحمولة في
  # `/tmp`، وتُقرَأ غالبًا لأمرٍ آخر: بُنِيت حمولةٌ جديدةٌ ولم تُثبَّت بصمتُها.
  # والمشغّلُ واقفٌ على الخادم بجذر، لا يملك المقيسة ولا يعرف السطرَ الذي يُحدَّث —
  # فيفعل أسوأَ شيء: يلصق ما يظنّه، أو يُعطّل الفحص. والحارسُ الذي لا يُطاع يُلتَفّ
  # عليه، فتُقال الحالتان وتُميَّزان.
  _got_tree="$(_sha "$STAGE/tree.tgz")"
  [[ "$_got_tree" == "$SHA_TREE" ]] || die "بصمةُ tree.tgz لا تطابق المثبَّتة في هذا السكربت.
   المعنى: هذه الشجرةُ ليست التي غادرت جهازَ البناء — إمّا لأنّها أحدثُ منها، وإمّا
           لأنّ أحدًا بدّلها في $STAGE بين الرفع والتركيب. والبصمةُ لا تفرّق، فتفرّق أنت.
   المثبَّتة: $SHA_TREE
   المقيسة : $_got_tree
   ▸ إن كنتَ رفعتَ هذه الحمولةَ للتوّ فهي جديدةٌ لم تُثبَّت بصمتُها بعد. والأصحُّ أن
     تُكتَب من جهاز البناء:  deploy/stage_payload.sh <مستخدم>@<خادم>
     ⚠️ ولا تُكتَب البصمةُ هنا بيدك: رأسُ هذا الملفّ يقول إنّ من يقدر على تبديل
     الحمولة يقدر على تبديل السطر الذي يحرسها — فسطرٌ جاهزٌ للّصق يُبطِل الحارس
     في اللحظة التي لا يُميَّز فيها بين الحالتين.
   ▸ وإن لم ترفع شيئًا جديدًا فهذا **ما تحرسه البصمة** — لا تُحدِّثها:
       sudo rm -rf $STAGE   ثمّ أعِد الرفعَ من جهاز البناء."
...
rollback() {
  ...
  if (( hurt == 0 )) && verify_serving >/dev/null 2>&1; then
    # ⚠️ **ولا يُشهَد بشجرةٍ سابقةٍ قد لا توجد.** إرجاعُ الشجرة و`$LIVE.bad` كلاهما
    #    مشروطٌ بـ`-d $OLD` أعلاه، فأوّلُ نشرٍ على خادمٍ نظيفٍ يسقط فيه القياسُ
    #    يُنتج تراجعًا **لا شجرةَ فيه**: ما يخدم هو المرفوضةُ نفسُها، ولا `.bad`
    #    أُنشئت. والصياغةُ المبهمةُ القديمة كانت تحتمل ذلك، والصريحةُ تُثبِت واقعةً
    #    خاطئة — وخطأٌ صريحٌ أسوأُ من إبهام.
    if [[ $NGINX_ONLY == no && -d $LIVE.bad ]]; then
      echo "↩️ عادت الكتلةُ والشجرةُ السابقتان — لم يسقط شيء." >&2
      echo "   والشجرةُ المرفوضةُ محفوظةٌ في $LIVE.bad — افحصها قبل حذفها." >&2
    else
      echo "↩️ عادت الكتلةُ السابقة، والشجرةُ المنشورةُ كما هي — الموقعُ يخدم." >&2
    fi
    echo "   السببُ المباشر: $why" >&2
    echo "   قبل إعادة المحاولة:  sudo nginx -t" >&2
  else
...
if ! verify_cache; then
  # «نافذةُ عمًى» مصطلحٌ داخليّ. ومن يقرأ هذا بعد سنةٍ يرى موقعًا **يخدم بالكامل**
  # يُتراجَع عنه بسبب ترويسة — قرارٌ صحيحٌ يحتاج تبريرَه في اللحظة، ووِجهةً للفحص.
  echo "   ⛔ ترويسةُ /out/ ليست ما في الكتلة (المقيسة: '$(_cc /out/nls.messages.js)')." >&2
  echo "      المعنى: زائرٌ عائدٌ يبقى على حزمةٍ قديمةٍ لا يبلغها نشرٌ ولا إصلاحٌ أمنيّ." >&2
  echo "      الأرجح: كتلةٌ أسبقُ في sites-enabled تلتقط /out/، أو add_header في مستوًى أدنى" >&2
  echo "              يُلغي وراثةَ ما فوقه." >&2
  echo "      افحص:  sudo nginx -T | grep -n -A6 'location /out/'" >&2
  rollback "ترويسةُ التخزين على /out/ ليست ما في الكتلة"
fi

echo
echo "   الشجرةُ السابقة في $OLD — احذفها بعد التأكّد:  sudo rm -rf $OLD"
# ‏تعليقُ رأسِ الملفّ يُقرأ مرّةً، ورسالةُ الخرج تُقرأ كلَّ مرّة. والنسيانُ هنا يوقف
# النشرَ القادمَ برسالةٍ تبدو اتّهامًا بالتلاعب.
echo "   ⚠️ والبصمتان في هذا السكربت تخصّان هذه الحمولة — الحمولةُ القادمة تحتاج"
echo "      غيرَهما، ويكتبهما  deploy/stage_payload.sh  من جهاز البناء."
