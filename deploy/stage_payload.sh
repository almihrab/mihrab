...
# ‏**والتحليلُ بعد `die` لا قبلها.** أوّلُ صياغةٍ وضعت الكتلةَ فوق تعريفها، فكان
# رفضُ الوسيط المجهول يسقط بـ`die: command not found` — أي أنّ الحارسَ الذي كُتِب
# ليقول «وسيطٌ غيرُ معروف» يقول شيئًا آخر، والوسيطُ يمرّ. قِيس بتشغيله.
# **ووسيطٌ غيرُ معروفٍ يُرفَض ولا يُهمَل** — كنظيرتها في `static_deploy.sh`، والتناظرُ
# في الاسم بلا تناظرٍ في الصرامة هو الفخُّ عينُه الذي وُضِعت الرايةُ لتفاديه:
# ‏`--nginx_only` أو `-n` كان يمرّ فيصير **وجهةَ رفعٍ** (`DEST="--nginx_only"`)، فتُحزَم
# ‏≈203 م.ب ويُنادى `ssh -p 2000 --nginx_only`. ورايةٌ بعد الوجهة كانت تُهمَل صامتة.
NGINX_ONLY=no
case "${1:-}" in
  "")           ;;
...
(( $# <= 1 )) || die "وسائطُ زائدة: $* — [--nginx-only] ثمّ وجهةٌ واحدةٌ على الأكثر
   (والرايةُ تسبق الوجهةَ ولا تليها)"
DEST="${1:-}"
